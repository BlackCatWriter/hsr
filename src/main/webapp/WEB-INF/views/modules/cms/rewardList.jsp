...
$(document).ready(function() {
    $("#btnExport").click(function() {
        ...
        function(v, h, f) {
            if (v == "ok") {
                $("#searchForm").attr("action", "${ctx}/cms/reward/export");
                $("#searchForm").submit();
            }
        },
        {
...
function page(n, s) {
    $("#pageNo").val(n);
    $("#pageSize").val(s);
    $("#searchForm").attr("action", "${ctx}/cms/reward/search");
    $("#searchForm").submit();
    return false;
}
</script>
...
<body>
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/cms/reward/import" method="post" enctype="multipart/form-data"
			...
			<br /> <input id="btnImportSubmit" class="btn btn-primary" type="submit" value="   导    入   " /> <a
				href="${ctx}/cms/reward/import/template">下载模板</a>
		</form>
	</div>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cms/reward">项目列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="reward" action="${ctx}/cms/reward/search" method="post"
