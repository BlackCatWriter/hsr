...
function showDetails(dataId) {
	$.getJSON("${ctx}/oa/reward/detail/" + dataId,
            function(data) {
...
function save() {
    $("#checkForm").attr("action", "${ctx}/oa/reward/saveRewardToUser");
    $("#checkForm").submit();
}

function saveAndSubmit() {
    $("#checkForm").attr("action", "${ctx}/oa/reward/submitRewardToUser");
    $("#checkForm").submit();
}
function complete(taskId, variables) {
    ...
    function(resp) {
        if (resp == 'success') {
            top.$.jBox.tip('任务完成');
            redirectForm.action = "${ctx}/oa/reward";
            redirectForm.submit();
        } else {
            top.$.jBox.tip('操作失败!');
        }
...
<body>
	<ul class="nav nav-tabs">
			<li class="active"><a href="${ctx}/oa/reward/form">科技进步奖登记</a></li>
		<li><a href="${ctx}/oa/reward/list">所有任务</a></li>
		<li><a href="${ctx}/oa/reward/task">待办任务</a></li>
	</ul>
	<br />
	<form:form id="inputForm" modelAttribute="reward" action="${ctx}/oa/reward/save" method="post"
		...
		<c:if test="${form or modify}">
			<div class="control-group">
				<label class="control-label"><font color='red'>*</font>科技进步奖名称:</label>
				<div class="controls">
			...
			<div class='control-group'>
				<label class='control-label'>第一完成人年龄:</label>
				<div class='controls'>
		...
		<div class="control-group">
			...
			<div class="control-group">
				...
				<c:if test="${modify or audit or forAudit}">
					<div class="controls">
						已上传文件：<a href="${ctx}/oa/reward/get/${reward.id}">${reward.file}</a>
					</div>
				</c:if>
				<div class="controls">
					<input id="file" name="file" type="hidden" value="${reward.file}" /> <input id="fileupload" type="file"
						name="files[]" data-url="${ctx}/oa/reward/upload/reward">
				</div>
			</div>
			<div class="control-group">
				<div class="controls">
		...
		<c:if test="${audit eq true}">
			...
			<div class='control-group'>
				...
				<div class='controls'>
					<form:input id='pt_fee' path='pt_fee' type='text' class='number' />
				</div>
			</div>
			<div class='control-group'>
				<label class='control-label'>申请状态追踪:</label>
				<div class='controls'>
	...
	<c:if test="${forAudit eq true}">
		<form:form id="checkForm" modelAttribute="rewardToUser" action="${ctx}/oa/reward/saveRewardToUser" method="post"
			class="form-horizontal">
			...
			<div class="control-group">
				<label class="control-label">申请级别:</label>
				<div class="controls">
					<form:input id='grade' path='reward.grade' type='text' value="${fns:getDictLabel(reward.grade, 'reward_grade', '无')}"
						readonly="true" />
				</div>
			</div>
			<div class="control-group">
			...
			<div class="control-group">
				...
				<div class="controls">
					<a href="${ctx}/oa/reward/get/${reward.id}">${reward.file}</a>
				</div>
			</div>
			<div class="control-group">
				<label class="control-label">备注：</label>
			...
			<hr>
			<div class="control-group">
				<label class="control-label"><font color='red'>*</font>创新性：</label>
				<div class="controls">
			...
			<div class="control-group">
				<label class="control-label"><font color='red'>*</font>科学性：</label>
				<div class="controls">
			...
			<div class="control-group">
				<label class="control-label"><font color='red'>*</font>发表论文：</label>
				<div class="controls">
