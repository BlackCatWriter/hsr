...
$(document).ready(function() {
    $("#btnExport").click(function() {
        ...
        function(v, h, f) {
            if (v == "ok") {
                $("#searchForm").attr("action", "${ctx}/cms/newTecReward/export");
                $("#searchForm").submit();
            }
        },
        {
...
function page(n, s) {
    $("#pageNo").val(n);
    $("#pageSize").val(s);
    $("#searchForm").attr("action", "${ctx}/cms/newTecReward/search");
    $("#searchForm").submit();
    return false;
}
</script>
...
<body>
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/cms/newTecReward/import" method="post" enctype="multipart/form-data"
			...
			<br /> <input id="btnImportSubmit" class="btn btn-primary" type="submit" value="   导    入   " /> <a
				href="${ctx}/cms/newTecReward/import/template">下载模板</a>
		</form>
	</div>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/cms/newTecReward">新技术引进奖列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="reward" action="${ctx}/cms/newTecReward/search" method="post"
		...
		<form:select path="delFlag">
			...
			<label>获奖年限：&nbsp;</label><input id="year" name="year" type="text" readonly="readonly" maxlength="20"
				...
				id="btnSubmit" class="btn btn-primary" type="button" value="查询" onclick="page()"/> &nbsp;<input id="btnExport"
		...
		<tbody>
			<c:forEach items="${page.list}" var="reward">
				<tr>
					...
					<td>${fns:getDictLabel(reward.grade, 'reward_grade', '无')}</td>
					...
					<td><a href="${ctx}/cms/newTecReward/form?id=${reward.id}">查看</a></td>
