...
import com.ndtl.yyky.modules.oa.dao.LeaveDao;
...
public class LeaveService extends BaseService {
	...
	public ProcessInstance save(Leave entity, Map<String, Object> variables) {
		...
		ProcessInstance processInstance = runtimeService
				.startProcessInstanceByKey(ProcessDefinitionKey.Leave.getKey(),
						businessKey, variables);
		String processInstanceId = processInstance.getId();
		entity.setProcessInstanceId(processInstanceId);
		leaveDao.updateProcessInstanceId(entity.getId(),
				entity.getProcessInstanceId());
		logger.debug(
				"start process of {key={}, bkey={}, pid={}, variables={}}",
				new Object[] { ProcessDefinitionKey.Leave.getKey(),
	...
	public List<Leave> findTodoTasks(String userId) {
		...
		List<Task> todoList = taskService.createTaskQuery()
				.processDefinitionKey(ProcessDefinitionKey.Leave.getKey())
	...
	public Page<Leave> find(Page<Leave> page, Leave leave) {
		...
		if (StringUtils.isNotBlank(leave.getIds())) {
			String ids = leave.getIds().trim().replace("　", ",")
					.replace(" ", ",").replace("，", ",");
		...
		for (Leave item : result.getList()) {
			...
			if (processInstanceId == null
					|| processInstanceId.trim().length() == 0) {
				continue;
			}
