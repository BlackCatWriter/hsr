...
<head>
	...
	<script type="text/javascript">
		...
		function academicChange(academicId){
			location.href='${ctx}/oa/academic/cost?id=' + academicId;
		}
		...
		function complete(taskId, variables) {
		    ...
		    $.post('${ctx}/oa/workflow/complete/' + taskId, {
				...
				if (resp == 'success') {
					top.$.jBox.tip('任务完成');
					inputForm.action = "${ctx}/oa/academic";
					inputForm.submit();
				} else {
					top.$.jBox.tip('操作失败!');
				}
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/oa/academic/cost">学术活动经费报销</a></li>
		<li><a href="${ctx}/oa/academic/list">所有任务</a></li>
		<li><a href="${ctx}/oa/academic/task">待办任务</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="academiccost" action="${ctx}/oa/academic/savecost" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:select path="academic.id" class="required"  onchange="academicChange(this.value);">
				<form:option value=""></form:option>
				<c:forEach items="${academicList}" var="selectedacad">
					<c:if test="${selectedacad.id eq selectedId}">
						<form:option value="${selectedacad.id}" selected="selected">${selectedacad.academicName }</form:option>
					</c:if>
					<c:if test="${selectedacad.id != selectedId}">
						<form:option value="${selectedacad.id}">${selectedacad.academicName}</form:option>
					</c:if>
				</c:forEach>
				</form:select>
			</div>
