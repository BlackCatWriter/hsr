...
<body>
	<form:form id="searchForm" modelAttribute="achieve" action="${ctx}/cms/achieve/list" method="post"
		...
		<c:if test="${not fns:isManager()}"> 
			...
			<c:if test="${fns:isManager()}"> 
				<tags:nameSuggest value="${achieve.weightBelong}" labelValue="${achieve.weightBelongDisplayName}" cssClass="required"
		...
		<div style="margin-top: 8px;">
			<label>年份：</label> <input id="searchYear" name="searchYear" type="text" readonly="readonly" maxlength="20"
				class="input-medium Wdate required" style="width: 163px;" value="${achieve.searchYear}"
	...
	<div class="control-group">
		...
		<div class="controls">
			<label>姓名：</label> <input value="${achieve.weightBelongUser.name}" readonly="readonly" /> <label>所属科室：</label> <input
				value="${achieve.weightBelongUser.office.name}" readonly="readonly" /> <label>职称：</label> <input
				value="${fns:getDictLabel(achieve.weightBelongUser.title,'userTitle','')}" readonly="readonly" />
		</div>
		<c:if test="${!empty achieve.thesisList}">
		...
		<div class="controls">
			<table id="contentTable" class="table table-striped table-bordered table-condensed">
				...
				<tbody>
					<c:forEach items="${achieve.thesisList}" var="thesis">
						<tr>
							<td>${thesis.title}</td>
		...
		<c:if test="${!empty achieve.projectList}">
		...
		<div class="controls">
			<table id="contentTable" class="table table-striped table-bordered table-condensed">
				...
				<tbody>
					<c:forEach items="${achieve.projectList}" var="project">
						<tr>
		...
		<c:if test="${!empty achieve.newTecRewardList}">
		...
		<div class="controls">
			<table id="contentTable" class="table table-striped table-bordered table-condensed">
				...
				<tbody>
					<c:forEach items="${achieve.newTecRewardList}" var="reward">
						<tr>
		...
		<c:if test="${!empty achieve.rewardList}">
		...
		<div class="controls">
			<table id="contentTable" class="table table-striped table-bordered table-condensed">
				...
				<tbody>
					<c:forEach items="${achieve.rewardList}" var="reward">
						<tr>
							<td>${reward.rewardName}</td>
							<td>${reward.rewardGrade}</td>
							<td>${reward.rewardLevel}</td>
		...
		<c:if test="${!empty achieve.tecAdvrewardList}">
		...
		<div class="controls">
			<table id="contentTable" class="table table-striped table-bordered table-condensed">
				...
				<tbody>
					<c:forEach items="${achieve.tecAdvrewardList}" var="reward">
						<tr>
							<td>${reward.rewardName}</td>
							<td>${reward.rewardGrade}</td>
		...
		<c:if test="${!empty achieve.bookList}">
		...
		<div class="controls">
			<table id="contentTable" class="table table-striped table-bordered table-condensed">
				...
				<tbody>
					<c:forEach items="${achieve.bookList}" var="book">
						<tr>
							<td>${book.title}</td>
		...
		<c:if test="${!empty achieve.patentList}">
		...
		<div class="controls">
			<table id="contentTable" class="table table-striped table-bordered table-condensed">
				...
				<tbody>
					<c:forEach items="${achieve.patentList}" var="patent">
						<tr>
							<td>${patent.title}</td>
							<td>${patent.number}</td>
							<td>${patent.time}</td>
