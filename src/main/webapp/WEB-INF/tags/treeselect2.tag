...
<script type="text/javascript">
	$("#${id}Button").click(function(){
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/tag/treeselect2?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&selectIds="+$("#${id}Id").val(), "选择${title}", 300, 420, {
			buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				if (v=="ok"){
					...
					for(var i=0; i<nodes.length; i++) {//<c:if test="${checked}">
						...
						if(!nodes[i].isParent && !reg.test(nodes[i].id))
						{
							ids.push(nodes[i].id);
						...
						var tempName;
						   for(var i=0;i<ids.length;i++)
						    {
						        for(var j=0;j<i;j++)
						        {
						            if(parseInt(ids[j])>parseInt(ids[i]))
						            {
						                temp=ids[j];
						                ids[j]=ids[i];
						                ids[i]=temp;
						   ...
						   for(var i=0;i<ids.length;i++)
						   {
						        ids[i]=ids[i].substring(ids[i].indexOf('_'));
						   }
						$("#${id}Id").val(ids);
						$("#${id}Name").val(names);
