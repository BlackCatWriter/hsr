...
$(document).ready(function() {
    ...
    $(".handle").click(function() {
        ...
        if (tkey == "kjDeptAudit") {
            $.getJSON("${ctx}/oa/newTecReward/detail/" + rewardId,
            function(data) {
                var html = Mustache.render($("#auditTemplate").html(), data);
                top.$.jBox(html, {
                    title: "流程办理[" + obj.data("tname") + "]",
        ...
        if (tkey == "lxAudit") {
            $.getJSON("${ctx}/oa/newTecReward/detail/" + rewardId+"?time="+new Date().getTime(),
            function(data) {
                ...
                top.$.jBox(html, {
                    ...
                    submit: function(v, h, f) {
                        //同意
                        if (v == "yes") {
                            top.$.jBox("<div style='padding:10px;'>" + "<div class='control-group'>"  + "<div class='controls'>"+ "<label class='control-label'>奖励单位:</label>" + "<input id='approvalOrg' name='approvalOrg' type='text' />" + "</div>" + "</div>" 
        ...
        //调整申请
        if (tkey == "modifyApply") {
        	redirectForm.action = "${ctx}/oa/newTecReward/formModify/" + rewardId + "/" + taskId;
        	redirectForm.submit();
        }
        //院内评审
        if (tkey == "hosAudit") {
        	redirectForm.action = "${ctx}/oa/newTecReward/formAudit/" + rewardId + "/" + taskId;
        	redirectForm.submit();
        }
        //院内专家审核
        if (tkey == "assignCheck") {
        	redirectForm.action = "${ctx}/oa/newTecReward/forAudit/" + rewardId;
        	redirectForm.submit();
        }
    });
});
...
function assign() {
    $("#assignForm").attr("action", "${ctx}/oa/newTecReward/assign");
    $("#assignForm").submit();
}

function editStatus() {
    $("#lxForm").attr("action", "${ctx}/oa/newTecReward/editStatus");
    $("#lxForm").submit();
}
...
<script type="text/template" id="auditTemplate">
		...
		<table class="table table-striped ">
			...
			<tr>
				<td>新技术引进奖：</td>
				<td><a href="${ctx}/oa/newTecReward/get/{{id}}" target='_blank'>{{rewardName}}</a></td>
			</tr>
			<tr>
				<td>第一完成人：</td>
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/oa/newTecReward/task">待办任务</a></li>
	</ul>
